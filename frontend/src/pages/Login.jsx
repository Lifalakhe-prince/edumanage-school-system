...
const Login = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    localStorage.setItem("edu_user", JSON.stringify({ username, role }));
    if (role === "teacher") {
      navigate("/teachers");
    } else if (role === "admin") {
      navigate("/admin");
  ...
  return (
    <div style={{
      ...
      <p style={{
        ...
        Don't have an account? <a href="/register" style={{ color: "#228B22", textDecoration: "none" }}>Register here</a>
      </p>
      <style>{`
        @keyframes fadeIn {
          from { opacity: 0; }
